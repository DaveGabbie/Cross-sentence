...
class RESIDE(Model):
    ...
    def add_model(self):
        ...
        with tf.variable_scope('self_attention_lstm') as scope:
            h = self.self_attention(lstm_out,self.p.num_units)

        with tf.variable_scope('interaction') as scope:
            weight=tf.Variable(tf.random_normal([lstm_out_dim, lstm_out_dim]), name='aspect_w')

            bias = tf.Variable(tf.zeros([lstm_out_dim]), name='aspect_b')

            aspect_avg = tf.reduce_mean(de_out, 1)

            #print('aspect_avg.get_shape():' ,aspect_avg.shape)

            context_att = tf.nn.softmax(tf.nn.tanh(tf.einsum('ijk,kl,ilm->ijm', h, weight,
                                                         tf.expand_dims(aspect_avg, -1)) + bias))
...
if __name__== "__main__":
    ...
    # parser.add_argument('-drop',	 dest="dropout", 	default=0.4,  			type=float,			help='Dropout for full connected layer')
    # parser.add_argument('-rdrop',	 dest="rec_dropout", 	default=0.4,  			type=float,			help='Recurrent dropout for LSTM')
 ...
 #   parser.add_argument('-eps', 	 dest="eps", 		default=0,  		type=float, 			help='Value of epsilon')
